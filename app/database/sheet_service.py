...
from sqlalchemy import select, update as sqlalchemy_update_stmt, delete as sqlalchemy_delete_stmt, Float
...
from .models import (
    ...
    SqliteBase, PendingSheetOperation  # Теперь снова используем ORM модель для SQLite
)
from config import (
    EXPECTED_SHEET_TITLES, CACHE_REFRESH_INTERVAL_SECONDS,
    SQLITE_DB_PATH,  # PENDING_OPERATIONS_TABLE_NAME (имя таблицы берется из __tablename__ модели)
    QUEUE_WORKER_INTERVAL_SECONDS, QUEUE_WORKER_MAX_ATTEMPTS
)
...
class AsyncSheetServiceWithQueue:
    def __init__(self, sheet_url: str, credentials_path: str, loop: Optional[asyncio.AbstractEventLoop] = None):
        ...
        if not os.path.exists(self.gsheet_credentials_path):
            raise FileNotFoundError(f"GSheet credentials file not found at: {self.gsheet_credentials_path}")
        ...
        self.gsheet_model_map: Dict[str, Type[GSheetBase]] = {
            "Товары": Product, "Заказы": Order, "Тип доставки": DeliveryType,
            "Настройка платежей": PaymentSetting, "Рассылки": Mailing,
        }

        self.gsheet_catalog = self._build_gsheet_catalog_sync()
        if not self.gsheet_catalog:
            logger.warning("GSheet catalog is empty. Data fetching from GSheets might fail.")
        # Синхронный движок для Shillelagh (Google Sheets)
        self.gsheet_db_engine = create_sync_engine(
            "gsheets://", service_account_file=self.gsheet_credentials_path, catalog=self.gsheet_catalog,
        )
        self.GSheetSessionLocal = sessionmaker(
            autocommit=False, autoflush=False, bind=self.gsheet_db_engine, class_=SyncSqlAlchemySession
        )

        # --- Асинхронный движок и фабрика сессий для SQLite ---
        self.sqlite_db_url = f"sqlite+aiosqlite:///{SQLITE_DB_PATH}"
        self.sqlite_async_engine = create_async_engine(self.sqlite_db_url)  # echo=True для отладки SQL
    ...
    async def _ensure_sqlite_tables_exist(self):
        ...
        logger.info(f"Ensured SQLite tables (defined in SqliteBase) exist at {SQLITE_DB_PATH}.")
    ...
    def _build_gsheet_catalog_sync(self) -> Dict[str, str]:  # Same
        catalog = {}
        try:
            gspread_client = gspread.service_account(filename=self.gsheet_credentials_path)
            ...
            for expected in self.expected_gsheet_titles:
                if expected not in catalog: logger.warning(
    ...
    def _get_gsheet_model_by_alias_sync(self, sheet_alias: str) -> Type[GSheetBase]:  # Same
        model_class = self.gsheet_model_map.get(sheet_alias)
        if not model_class: raise ValueError(f"GSheet ORM model for alias '{sheet_alias}' not found.")
        if sheet_alias not in self.gsheet_catalog: raise ValueError(f"GSheet alias '{sheet_alias}' not in catalog.")
        return model_class

    def _gsheet_row_to_dict_sync(self, row_object: GSheetBase, model_class: Type[GSheetBase]) -> Dict[str, Any]:  # Same
        if row_object is None: return {}
        data_dict = {}
        for col_attr in sqlalchemy_inspect(model_class).mapper.column_attrs:
            data_dict[col_attr.key] = getattr(row_object, col_attr.key)
        return data_dict

    def _fetch_single_gsheet_data_blocking(self, sheet_alias: str) -> List[Dict[str, Any]]:  # Same
        ...
        try:
            ...
            logger.error(f"(Sync) Error fetching GSheet data for '{sheet_alias}': {e}", exc_info=True);
            return []
        finally:
            gsheet_session.close()

    def _gsheet_create_row_blocking(self, sheet_alias: str, data_payload: dict) -> Optional[dict]:  # Same logic
        model_class = self._get_gsheet_model_by_alias_sync(sheet_alias)
        gsheet_session: SyncSqlAlchemySession = self.GSheetSessionLocal()
        try:
            valid_data = {};
            model_attrs = {col.key for col in sqlalchemy_inspect(model_class).mapper.column_attrs}
            for key, value in data_payload.items():
                if key in model_attrs: valid_data[key] = value
            ...
            logger.error(f"(Sync) GSheet create_row error '{sheet_alias}': {e}", exc_info=True);
            return None
        finally:
            gsheet_session.close()

    def _gsheet_update_rows_blocking(self, sheet_alias: str, filter_criteria: dict,
                                     new_data: dict) -> int:  # Same logic
        model_class = self._get_gsheet_model_by_alias_sync(sheet_alias)
        gsheet_session: SyncSqlAlchemySession = self.GSheetSessionLocal();
        updated_count = 0
        try:
            ...
            for record in records_to_update:
                ...
                updated_count += 1
            if updated_count > 0: gsheet_session.commit()
            return updated_count
        except Exception as e:
            gsheet_session.rollback();
            logger.error(f"(Sync) GSheet update_rows error '{sheet_alias}': {e}", exc_info=True);
            return 0
        finally:
            gsheet_session.close()

    def _gsheet_delete_rows_blocking(self, sheet_alias: str, filter_criteria: dict) -> int:  # Same logic
        model_class = self._get_gsheet_model_by_alias_sync(sheet_alias)
        gsheet_session: SyncSqlAlchemySession = self.GSheetSessionLocal();
        deleted_count = 0
        try:
            records_to_delete = gsheet_session.query(model_class).filter_by(**filter_criteria).all()
            for record in records_to_delete:
                gsheet_session.delete(record);
                deleted_count += 1
            if deleted_count > 0: gsheet_session.commit()
            return deleted_count
        except Exception as e:
            gsheet_session.rollback();
            logger.error(f"(Sync) GSheet delete_rows error '{sheet_alias}': {e}", exc_info=True);
            return 0
        finally:
            gsheet_session.close()
    ...
    async def _populate_in_memory_cache_for_sheet(self, sheet_alias: str):  # Same
        ...
        if sheet_alias not in self.gsheet_model_map or sheet_alias not in self.gsheet_catalog:
            logger.warning(f"GSheet alias '{sheet_alias}' invalid or not in catalog. Skipping cache population.")
            return
        data = await asyncio.to_thread(self._fetch_single_gsheet_data_blocking, sheet_alias)
        async with self._cache_lock:
            self._in_memory_cache[sheet_alias] = data
            self._in_memory_cache_last_updated[sheet_alias] = time.monotonic()
        logger.info(f"In-memory cache populated for GSheet '{sheet_alias}', {len(data)} rows.")

    async def _populate_all_in_memory_caches(self):  # Same
        logger.info("Populating all in-memory caches from GSheets...")
        tasks = [self._populate_in_memory_cache_for_sheet(alias) for alias in self.gsheet_model_map.keys() if
                 alias in self.gsheet_catalog]
    ...
    async def _periodic_gsheet_cache_refresh_task(self):  # Same
        ...
        while not self._is_shutting_down.is_set():
            try:
                ...
                logger.info("Periodic GSheet cache refresh task cancelled."); break
            except Exception as e:
                logger.error(f"Error in periodic GSheet cache refresh: {e}", exc_info=True); await asyncio.sleep(60)

    async def force_gsheet_in_memory_cache_refresh(self, sheet_alias: Optional[str] = None):  # Same
        if sheet_alias:
            if sheet_alias not in self.gsheet_model_map or sheet_alias not in self.gsheet_catalog:
                logger.warning(f"Cannot refresh GSheet cache for '{sheet_alias}': invalid or not in catalog.");
                return
    ...
    # ОНИ ОСТАЮТСЯ БЕЗ ИЗМЕНЕНИЙ
    async def get_data_from_cache(self, sheet_alias: str) -> List[Dict[str, Any]]:  # Same
        await self._initial_gsheet_cache_populated.wait()
        if sheet_alias not in self.gsheet_model_map or sheet_alias not in self.gsheet_catalog:
            logger.warning(f"'{sheet_alias}' not configured or found for cache read.")
            if sheet_alias not in self._in_memory_cache and sheet_alias in self.gsheet_model_map and sheet_alias in self.gsheet_catalog:
                logger.info(f"Cache miss for {sheet_alias} after init, attempting one-time GSheet population.")
                await self._populate_in_memory_cache_for_sheet(sheet_alias)
        async with self._cache_lock:
            return list(self._in_memory_cache.get(sheet_alias, []))

    async def read_rows_from_cache(  # Same logic
            self, sheet_alias: str, filter_criteria: Optional[Dict[str, Any]] = None,
            order_by_attributes: Optional[List[str]] = None, row_limit: Optional[int] = None,
            row_offset: Optional[int] = None,
    ) -> List[Dict[str, Any]]:
        sheet_data = await self.get_data_from_cache(sheet_alias)
        if not sheet_data: return []
        if filter_criteria: sheet_data = [row for row in sheet_data if
                                          all(row.get(k) == v for k, v in filter_criteria.items())]
        if order_by_attributes:
            for attr_name in reversed(order_by_attributes):
                ...
                if sheet_data and attr_actual not in sheet_data[0]: logger.warning(
                    f"Sort key '{attr_actual}' not in cached '{sheet_alias}'"); continue
                try:
                    sheet_data.sort(key=lambda x: x.get(attr_actual), reverse=is_desc)
                except TypeError:
                    logger.warning(f"TypeError sorting '{sheet_alias}' by '{attr_actual}'.")
        if row_offset: sheet_data = sheet_data[row_offset:]
        if row_limit: sheet_data = sheet_data[:row_limit]
        return sheet_data
    ...
    async def _add_operation_to_sqlite_queue_orm(  # ИЗМЕНЕНО: на SQLAlchemy Async ORM
            self, sheet_alias: str, operation_type: str,
            filter_criteria: Optional[dict] = None, data_payload: Optional[dict] = None
    ) -> int:
        op_id = -1
            ...
            async with sqlite_session.begin():  # Начинаем транзакцию
                try:
                    pending_op = PendingSheetOperation(
                        ...
                        filter_criteria_json=json.dumps(filter_criteria) if filter_criteria else None,
                        data_payload_json=json.dumps(data_payload) if data_payload else None,
                        status="pending",
                    ...
                    logger.error(f"Error adding operation to SQLite queue (SQLAlchemy ORM): {e}", exc_info=True)
    ...
    async def _optimistically_update_in_memory_cache(  # Остается как есть (async)
        ...
        async with self._cache_lock:
            if sheet_alias not in self._in_memory_cache: logger.warning(
                f"Optimistic: cache for '{sheet_alias}' not found."); return
            current_data = self._in_memory_cache.get(sheet_alias, []);
            op = operation_type.upper()
            if op == "CREATE" and data_payload:
                current_data.append(data_payload.copy()); logger.debug(f"Optimistic CREATE cache '{sheet_alias}'")
            elif op == "UPDATE" and filter_criteria and data_payload:
                updated_c = 0
                for i, row in enumerate(current_data):
                    if all(row.get(k) == v for k, v in filter_criteria.items()): current_data[i] = {**row,
                                                                                                    **data_payload}; updated_c += 1
                logger.debug(f"Optimistic UPDATE cache '{sheet_alias}': {updated_c} affected.")
                ...
                logger.debug(f"Optimistic DELETE cache '{sheet_alias}': {deleted_c} removed.")
            else:
                logger.warning(f"Unknown op '{operation_type}' for optimistic cache update.")

    async def create_row(self, sheet_alias: str, data_payload: dict) -> Optional[dict]:
        op_id = await self._add_operation_to_sqlite_queue_orm(sheet_alias, "CREATE", data_payload=data_payload)
        if op_id == -1: return None
        await self._optimistically_update_in_memory_cache(sheet_alias, "CREATE", data_payload=data_payload)
        logger.info(f"CREATE op for '{sheet_alias}' queued (ID: {op_id}). Optimistic cache update done.")
        return data_payload

    async def update_rows(self, sheet_alias: str, filter_criteria: dict, new_data_payload: dict) -> int:
        op_id = await self._add_operation_to_sqlite_queue_orm(sheet_alias, "UPDATE", filter_criteria=filter_criteria,
                                                              data_payload=new_data_payload)
        if op_id == -1: return 0
        await self._optimistically_update_in_memory_cache(sheet_alias, "UPDATE", filter_criteria=filter_criteria,
                                                          data_payload=new_data_payload)
        logger.info(f"UPDATE op for '{sheet_alias}' queued (ID: {op_id}). Optimistic cache update done.")
        return 1

    async def delete_rows(self, sheet_alias: str, filter_criteria: dict) -> int:
        op_id = await self._add_operation_to_sqlite_queue_orm(sheet_alias, "DELETE", filter_criteria=filter_criteria)
        if op_id == -1: return 0
        await self._optimistically_update_in_memory_cache(sheet_alias, "DELETE", filter_criteria=filter_criteria)
        logger.info(f"DELETE op for '{sheet_alias}' queued (ID: {op_id}). Optimistic cache update done.")
        return 1

    # === SQLite Queue Processor (Background Worker) using SQLAlchemy Async ORM ===
    async def _process_pending_operations_task(self):
        await self._initial_gsheet_cache_populated.wait()
        logger.info("Starting SQLite pending operations processor task (SQLAlchemy Async ORM).")
        while not self._is_shutting_down.is_set():
            ...
            try:
                async with self.AsyncSqliteSessionLocal() as sqlite_session:
                    async with sqlite_session.begin():  # Начинаем транзакцию для выбора и обновления статуса
                        stmt = select(PendingSheetOperation) \
                            .where(PendingSheetOperation.status.in_(["pending", "retry"])) \
                        ...
                        operation_to_process.last_attempt_at = datetime.datetime.utcnow()
                    ...
                    criteria = json.loads(
                        operation_to_process.filter_criteria_json) if operation_to_process.filter_criteria_json else None
                    payload = json.loads(
                        operation_to_process.data_payload_json) if operation_to_process.data_payload_json else None

                    success = False;
                    result_info = None

                    if op_type == "CREATE" and payload:
                        result_info = await asyncio.to_thread(self._gsheet_create_row_blocking, sheet_alias, payload)
                        success = result_info is not None
                    elif op_type == "UPDATE" and criteria and payload:
                        result_info = await asyncio.to_thread(self._gsheet_update_rows_blocking, sheet_alias, criteria,
                                                              payload)
                        success = result_info > 0  # Предполагая, что >0 означает успех
                    elif op_type == "DELETE" and criteria:
                        result_info = await asyncio.to_thread(self._gsheet_delete_rows_blocking, sheet_alias, criteria)
                        success = result_info > 0  # Предполагая, что >0 означает успех
                    ...
                    async with self.AsyncSqliteSessionLocal() as sqlite_update_session:
                        async with sqlite_update_session.begin():
                            if success:
                                ...
                                op_to_delete = await sqlite_update_session.get(PendingSheetOperation, op_id_processed)
                                if op_to_delete: await sqlite_update_session.delete(op_to_delete)
                                # Коммит произойдет автоматически
                                # Обновляем кэш после успешной записи в GSheet
                                await self.force_gsheet_in_memory_cache_refresh(sheet_alias)
                            else:
                                ...
                                op_to_update = await sqlite_update_session.get(PendingSheetOperation, op_id_processed)
                                if op_to_update:
                                    op_to_update.error_message = error_msg
                                    if op_to_update.attempts >= QUEUE_WORKER_MAX_ATTEMPTS:
                                        op_to_update.status = "failed_max_attempts"
                                    else:
                                        op_to_update.status = "retry"
                                # Коммит произойдет автоматически
            except Exception as e:
                logger.error(f"Error in SQLite queue processor task (SQLAlchemy ORM): {e}", exc_info=True)
                    ...
                    try:
                        async with self.AsyncSqliteSessionLocal() as sqlite_err_session:
                            async with sqlite_err_session.begin():
                                op_to_mark_failed = await sqlite_err_session.get(PendingSheetOperation, op_id_processed)
                                if op_to_mark_failed:
                                    op_to_mark_failed.status = "failed_worker_error"
                                    op_to_mark_failed.error_message = str(e)[
                                                                      :1000]  # Ограничиваем длину сообщения об ошибке
                    except Exception as e_db_update:
                        logger.error(
                            f"Failed to mark operation {op_id_processed} as 'failed_worker_error' in SQLite: {e_db_update}")
                await asyncio.sleep(self._queue_worker_interval)  # Пауза перед следующей попыткой цикла

            if self._is_shutting_down.is_set(): break
        logger.info("SQLite pending operations processor task (SQLAlchemy ORM) stopped.")
    ...
    async def start_services(self):
        ...
        if not self.gsheet_catalog:
            logger.warning("GSheet catalog is empty. In-memory cache and GSheet interactions will be limited.")
            self._initial_gsheet_cache_populated.set()
        else:
            await self._populate_all_in_memory_caches()

        if self._gsheet_periodic_refresh_task is None or self._gsheet_periodic_refresh_task.done():
            self._gsheet_periodic_refresh_task = asyncio.create_task(self._periodic_gsheet_cache_refresh_task())
            logger.info("Background GSheet in-memory cache refresh task started.")

        if self._queue_processor_task is None or self._queue_processor_task.done():
            self._queue_processor_task = asyncio.create_task(self._process_pending_operations_task())
            logger.info("Background SQLite queue processor task (SQLAlchemy ORM) started.")

    async def close(self):
        logger.info("Closing AsyncSheetServiceWithQueue (SQLAlchemy async SQLite version)...")
        self._is_shutting_down.set()

        tasks_to_await = []
        if self._gsheet_periodic_refresh_task: tasks_to_await.append(self._gsheet_periodic_refresh_task)
        if self._queue_processor_task: tasks_to_await.append(self._queue_processor_task)

        for task in tasks_to_await:
            if task and not task.done():
                try:
                    task.cancel(); await asyncio.wait_for(task, timeout=5.0)
        ...
        if self.gsheet_db_engine: await asyncio.to_thread(self.gsheet_db_engine.dispose)
        if self.sqlite_async_engine: await self.sqlite_async_engine.dispose()  # Закрываем асинхронный движок SQLite
        logger.info("AsyncSheetServiceWithQueue (SQLAlchemy async SQLite version) closed.")